...
import { Search, Bell, LogOut, Briefcase, ChevronDown, Check, Building } from "lucide-react";
...
export const ClientHeader: React.FC<ClientHeaderProps> = ({
    ...
    if (saved) {
      try {
        ...
        setUser({
          ...
          email: parsed.email || "",
          role: parsed.role || "Client Contact",
          avatar:
            parsed.avatar ||
        ...
        console.warn("Failed to parse user session in ClientHeader:", e);
      }
    }
  }, []);
  ...
  return (
    <header className="h-16 shrink-0 bg-white px-8 flex items-center justify-between sticky top-0 z-20 pt-2 border-b border-slate-100 font-sans">
      ...
      <div className="flex items-center gap-4">
        <div className="relative w-72 md:w-80">
          ...
          <input
            ...
            placeholder="Search client tasks, files, or messages..."
            ...
            {showProjectMenu && (
              <div className="absolute left-0 mt-2 w-64 bg-white rounded-2xl p-2 shadow-xl border border-slate-200 z-50 animate-in fade-in slide-in-from-top-2">
                <div className="px-3 py-1.5 border-b border-slate-100 mb-1 flex items-center justify-between">
                  <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Assigned Client Projects</p>
      ...
      {/* Right Controls: Role Badge & Profile Dropdown */}
      <div className="flex items-center gap-4">
        <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-[#006858] border border-emerald-200/80 text-[11px] font-extrabold">
          ...
          >
            <div className="relative">
              <img
                ...
                alt={user.name}
                className="w-9 h-9 rounded-full bg-emerald-50 border border-slate-200/80 object-cover ring-2 ring-[#006858]/30"
              />
              <span
                className={cn(
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-60 bg-white rounded-2xl p-2 shadow-xl border border-slate-100 z-50 animate-in fade-in slide-in-from-top-2">
              <div className="px-3 py-2 border-b border-slate-100 mb-1">
                ...
                <span className="inline-block mt-1 text-[9px] font-black uppercase bg-emerald-100 text-[#006858] px-2 py-0.5 rounded-full">
                  Client Contact
                </span>
              </div>
