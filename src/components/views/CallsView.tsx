...
import { PhoneCall, Video, PhoneOff, Mic, MicOff, VideoOff, Users, Calendar, Plus, Building } from "lucide-react";
...
export const CallsView: React.FC = () => {
  ...
  const [activeCallHost, setActiveCallHost] = useState("Irfan Tariq");
  const [muted, setMuted] = useState(false);
  const [videoOn, setVideoOn] = useState(true);

  const projectClients = activeProject?.clients || [];
  const projectMembers = activeProject?.teamMembers || [];
  const allParticipants = [...projectClients, ...projectMembers];
      ...
      {inCall ? (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-[300px]">
            <div className="bg-slate-800 rounded-2xl relative overflow-hidden flex items-center justify-center border border-slate-700">
              <img
                src="https://api.dicebear.com/7.x/avataaars/svg?seed=Irfan"
                alt="Irfan Tariq"
                className="w-full h-full object-cover"
              />
              <span className="absolute bottom-3 left-3 bg-slate-900/80 backdrop-blur-xs px-2.5 py-1 rounded-lg text-xs font-bold text-white">
                Irfan Tariq (Host)
              </span>
            </div>
            ...
            {allParticipants.slice(0, 5).map((p) => (
              <div
                key={p.id}
                className="bg-slate-800 rounded-2xl relative overflow-hidden flex items-center justify-center border border-slate-700"
              >
                <img
                  src={p.avatar}
                  alt={p.name}
                ...
                <span className="absolute bottom-3 left-3 bg-slate-900/80 backdrop-blur-xs px-2.5 py-1 rounded-lg text-xs font-bold text-white flex items-center gap-1">
                  {p.type === "client" ? <Building className="w-3 h-3 text-emerald-400" /> : null}
                  {p.name} ({p.role})
                </span>
              </div>
            ))}
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            {allParticipants.length === 0 ? (
              <div className="modern-card rounded-3xl p-8 bg-white border border-slate-200/90 text-center space-y-2">
                ...
                <p className="font-bold text-slate-700 text-sm">No Project Members Assigned</p>
                <p className="text-xs text-slate-400">
                  Select or create a project with team members and clients to invite to video calls.
                </p>
              </div>
            ) : (
                      ...
                      <div>
                        <h4 className="font-bold text-sm text-[#0F172A] flex items-center gap-1.5">
                          ...
                          <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-full ${
                            p.type === "client" ? "bg-emerald-100 text-[#006858]" : "bg-slate-100 text-slate-600"
                          }`}>
                            {p.type}
                          </span>
                        </h4>
          ...
          <div className="space-y-4">
            ...
            <div className="modern-card rounded-3xl p-5 bg-white border border-slate-200/90 shadow-sm space-y-3 text-xs">
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-100">
                <div>
                  <p className="font-bold text-[#0F172A]">Irfan Tariq</p>
